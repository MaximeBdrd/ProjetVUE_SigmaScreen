...
#include "mchpGfxDrv.h"
//#include "..\includes\gfxtcon.h"
#include "Primitive.h"
...
void EPMP_Init(void)
{
	...
	*/
	...
	PMCON1bits.BUSKEEP = 0;     								// bus keeper is not used
		...
		#elif ((GFX_EPMP_CS1_MEMORY_SIZE) <= 0x160000)
			PMCON3 |= 0x0007;     								// PMA18, PMA17, PMA16 address lines are enabled
                #else // ((GFX_EPMP_CS1_MEMORY_SIZE) > 0x160000)
                        PMCON3 |= 0x000F;
		#endif	     					
...
void PutImage1BPPExt(SHORT left, SHORT top, void *image, BYTE stretch)
{
    ...
    // Get image header
    ExternalMemoryCallback(image, 2, sizeof(BITMAP_HEADER)-2, ((char*)&bmp)+2);
