...
extern void TouchStoreCalibration(short* pCalibration);
extern void TouchLoadCalibration(short* pCalibration);
extern void TouchCalHWGetPoints(void);

extern const WORD mchpTouchScreenVersion;

void 	TouchCheckForCalibration(short* pCalibration);
...
*
********************************************************************/
void TouchInit(short* pCalibration)//NVM_WRITE_FUNC pWriteFunc, NVM_READ_FUNC pReadFunc, NVM_SECTORERASE_FUNC pSectorErase, void *initValues)
{
	...
	if (pCalDataRead != NULL)
	{
		...
		{*/
			...
			TouchCheckForCalibration(pCalibration);
		//}

		// load the calibration values, to check if the store worked
		TouchLoadCalibration(pCalibration);
	//}
	//else
	//{
...
void TouchCheckForCalibration(short* pCalibration)
{
	...
	while(1)
	{
		...
		if ((TouchGetX() == -1) && (TouchGetY() == -1) && pCalibration != 0 && pCalibration[8] != 1)
			break;
		else
			count++;
		if (count == 10)
		{
			// do calibration
        	TouchCalibration();
        	TouchStoreCalibration(pCalibration);
        	break;
  		}      	
 	}  	
...
void TouchCalibration(void)
{
    ...
    static const XCHAR  scr2StrLn3[] = {'p','r','e','s','s','i','n','g',' ','o','n',' ','t','h','e',' ','s','c','r','e','e','n',0};
